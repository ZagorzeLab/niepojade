...
// Types & loaders
import type { BusLine, AlertData, Stop, TicketType, Discount } from "@/lib/types/bus-schedule"
...
function BusScheduleContent() {
  ...
  const [tickets, setTickets] = useState<{ ticketTypes: TicketType[]; discounts: Discount[] } | null>(null)
  const [alerts, setAlerts] = useState<AlertData[]>([])
  const [stops, setStops] = useState<Stop[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [currentView, setCurrentView] = useState<ViewType>("timetable")
  const [selectedStop, setSelectedStop] = useState<{ stopId: string; lineId: string } | null>(null)
  ...
  if (isLoading || !busData || !tickets) return <FullScreenLoader />
          ...
          <div className="grid grid-cols-4 gap-1 py-2 pb-safe">
            {NAV_ITEMS.map(btn => (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 px-3 py-2.5 rounded-lg transition-all ${currentView === btn.view
